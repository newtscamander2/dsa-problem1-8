...
\section*{Comment}
To find a real value at index $d$ in an infinite long increasing sequence of real values $A=\{x_1 < x_2<x_3...\}$, a two-phased approach with both exponential search and binary search is performed due to an unknown upper bound for $A$. For the created algorithm it is assumed that the maximum value of $A$ cannot be accessed, otherwise a single-phased binary search algorithm would be sufficient.
