...
The second phase of our search is a binary search between the last index $2^{k-1}$ and the current index from phase 1 $2^{k}$. The interval size is therefore $2^{k} - 2^{k-1}$. This can be simplified to $2^{k-1}$. From \textcite{SupplementaryADS} \textbf{Theorem 6} we know that the worst case optimality of binary search can be calculated from $\lceil\log_2{(n+1)}\rceil$ where $n = 2^{k-1}$. We know from phase one that $k = \lceil \log_2{d}\rceil$, and therefore $n = 2^{\lceil \log_2{d}\rceil-1}$
