...
%
% EXPONENTIAL SEARCH: PSEUDO CODE
%
The pseudo code for our exponential search can be written as
\begin{center}
	\begin{minipage}{8.8cm}
		\begin{algorithmic}
			\Procedure{ExponentialSearch}{$A, y$}

			\State{$i = 1$}
			\While{$A[i] < y$}
			\State{$i = 2 \cdot i$}
			\EndWhile
			\State{$low, high = i/2, i$} \Comment{$A[low]<y \leq A[high]$}
...
This algorithm works because our list $A$ is a sorted list. The while loop stops at the fist index $i$ with $y \leq A[i]$. The previous probe gave us $A[i/2] < y$. This means that $A[low] <y \leq A[high]$ for $low, high = i/2, i$. Because the exercise has given us $y >x_1 = A[1]$ then the loop body runs at least once. Hence $i \geq 2$ and $low=i/2 \geq 1 = $ valid index.

We have therefore with our algorithm found a subset of $A$ which is of finite length, and we can therefore use a search algorithm such as binary search, which is much more performant than linear search.
...
\begin{center}
	\begin{minipage}{8.8cm}
		\begin{algorithmic}
			\Procedure{BinarySearch}{$A,y,low,high$}
			\While{$high - low > 1$}
...
\end{center}
Our binary search algorithm works because we give it a finite sorted list $B$, together with the invariant $A[low] < y \leq A[high]$. In our if statement both branches preserve the invariant (which must be true und the entire program execution). $if A[mid] <y$ then $low = mid$ keeps $A[low] < y$. Otherwise $y \leq A[mid]$ such that $high = mid$ keeps $y \leq A[high]$. Every iteration halves $high-low$ (because of the exponent). The loop therefore ends with $high = low + 1$, e.g. $x_{high-1} < y \leq  x_{high}$ such that $high=d$, no matter if $y$ occurs in the sequence of real values $A$. If we tried to make a binary search in the original list it would not have been possible because we wouldn't have an index for $high$ variable to start with.
...
It follows that the efficiency of the algorithm is dependent on the index $d$ of our wanted element $y$.
The probe indices from phase one are $1 \rightarrow 2 \rightarrow 4 \rightarrow 8 \rightarrow ...$, i.e. $2^k$ where $k$ is the number of doublings.

Our algorithm checks for $y \leq A[2^k]$ and stops at the first such $k$, which means that $2^{k-1}<d \leq 2^k$, and we can therefore solve for $k$ using a logarithm $k-1 < \log_2{d}\leq k$.
The number of doublings given by index $d$ is therefore $\lceil \log_2{d}\rceil=k$. Phase 1 therefore performs $k+1$ comparisons, one per probe.

The second phase of our search is a binary search between the last index $2^{k-1}$ and the current index from phase 1 $2^{k}$.
The interval size is therefore $2^{k} - 2^{k-1}$.
This can be simplified to $2^{k-1}$.

Every iteration does perform exactly one comparison $A[mid] < y$, and then it halves the interval such that phase 2 does perform $\log_2{2^{k-1}} = k-1$ comparisons.

From \textcite{SupplementaryADS} \textbf{Theorem 6} any comparison based search among the first $2^k$ elements requires at least $\lceil\log_2{(2^k+1)}\rceil$ $=k+1$ comparisons in the worst case. This means that the total is roughly within a factor of two of optimål.

We can now combine our two equations into one equation of how many comparisons are performed as a function of $d$.

\begin{equation}
	f(d)=(k+1)+(k-1)=2k=2 \lceil \log_2{(d)} \rceil
\end{equation}
...
\noindent\begin{tcolorbox}[breakable, toprule at break=0pt,bottomrule at break=0pt, colback={blue!5!white}, colframe={black!70!white}, arc={0mm}]
	To verify that our function for calculating how many iterations there are is correct, we'll go through a simple example of the algorithm.
