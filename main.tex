...
A simple (and quite effective) solution is to split the problem into two phases phase 1) determining a subset $B$ so $B \subseteq A$ using a "doubling" technique and phase 2) doing binary search on $B$.



\subsection*{Phase 1. Exponential search / doubling}
In this phase we find the subset of A in which y is located. This is done by doubling a search index $z$ until value of element at index $z$ is greater or equal to our desired value $y$.
%Rewrite this
...
\begin{center}
	...
	%These lines are really weird / show up wrong sometimes...
	\begin{tikzpicture}[ remember picture, overlay]
		\draw [->,red] ([yshift=-2ex]pic cs:a) [bend left] to ([yshift=-2ex]pic cs:b);
		\draw [->,red] ([yshift=-2ex]pic cs:b) [bend left] to ([yshift=-2ex]pic cs:d);
		\draw [->,red] ([yshift=-2ex]pic cs:d) [bend left] to ([yshift=-2ex]pic cs:h);

	\end{tikzpicture}
\end{center}

The pseudo code for our doubling search can we written like this:
\begin{center}
